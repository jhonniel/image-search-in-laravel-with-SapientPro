...
class WelcomeController extends Controller
{
    ...
    private function formatItemGroup($itemGroup)
    {
        ...
        // Ensure image_path is always a string (not null)
        $imagePath = $imagePath ?? '';
        
        // Get detected objects
        $detectedObjects = [];
        if ($firstItem->detected_objects) {
            if (is_string($firstItem->detected_objects)) {
                $decoded = json_decode($firstItem->detected_objects, true);
                $detectedObjects = $decoded !== null ? $decoded : [];
            } elseif (is_array($firstItem->detected_objects)) {
        ...
        foreach ($detectedObjects as $obj) {
            ...
            if (!empty($objName) && !in_array(strtolower($objName), $seenNames)) {
                $seenNames[] = strtolower($objName);
                $uniqueObjects[] = is_array($obj) ? $obj : ['name' => $obj, 'score' => 0.0];
            }
        }
        $detectedObjects = array_slice($uniqueObjects, 0, 5);
