                ...
                <div class="flex items-center justify-between gap-3">
                    ...
                    <div class="flex items-center space-x-2 sm:space-x-4">
                        ...
                        <div class="relative" x-data="{ open: false, unread: 0 }" @click.away="open = false">
                            <button class="relative p-2 text-gray-600 hover:text-purple-primary" @click="open = !open; if(open){ fetch('/api/notifications').then(r=>r.json()).then(d=>{ unread = d.unread; const list = document.getElementById('notif-list'); list.innerHTML = ''; (d.notifications||[]).forEach(n=>{ const li = document.createElement('div'); li.className='px-4 py-2 hover:bg-gray-50'; li.innerHTML = `<div class=\'text-sm font-medium text-gray-900\'>${n.title}</div><div class=\'text-xs text-gray-500\'>${n.message ?? ''}</div>`; list.appendChild(li); }); }); }">
                            ...
                            <div x-show="open" class="absolute right-0 mt-2 w-72 bg-white rounded-lg shadow-lg border border-gray-200 py-2 z-50" style="display:none;">
                                <div class="px-4 py-2 border-b flex items-center justify-between">
                                    ...
                                    <button class="text-xs text-purple-primary hover:underline" onclick="fetch('/api/notifications/mark-read',{method:'POST', headers:{'X-CSRF-TOKEN': document.querySelector('meta[name=csrf-token]').content}}).then(()=>{document.querySelectorAll('[x-data] [x-text]').forEach(()=>{});})">Mark all read</button>
                                </div>
